...
```{r libraries, echo=TRUE,fig.height=6,fig.width=8, cache=TRUE}
library(dplyr)
library(lubridate)
library(dygraphs)
library(XML)
...
for (url in master){
  ...
  if ("prcocs" %in% colnames(rawdata[[n]])){
    rawdata[[n]] <- rawdata[[n]] %>% filter(prcocs=="T    ")
  }
  if ("phcocs" %in% colnames(rawdata[[n]])){
    rawdata[[n]] <- rawdata[[n]] %>% filter(phcocs=="T    ") %>% rename(prcocs=phcocs)
  }
...
top.autores <- rawdata.df %>% filter(autor!="") %>% group_by(autor) %>% summarise(total=100*n()/nrow(rawdata.df)) %>% arrange(-total)
head(top.autores)
```